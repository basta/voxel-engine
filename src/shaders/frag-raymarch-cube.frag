...
float dist(vec3 p){
    return min(
        ...
        sphereDist(p, vec3(0.6, 0.5, 0.7), 0.1)
        )
    );
}
...
vec3 normal(vec3 p){
    ...
    && sphereDist(p, vec3(0.5), 0.1) < sphereDist(p, vec3(0.6, 0.5, 0.7), 0.1)
    ){
        return (vec3(0.5) - p)/distance(p, vec3(0.5));
    } else if (sphereDist(p, vec3(0.1, 0.5, 0.5), 0.1) < sphereDist(p, vec3(0.6, 0.5, 0.7), 0.1)) {
        return (vec3(0.1, 0.5, 0.5) - p)/distance(p, vec3(0.1, 0.5, 0.5));
    } else {
        return (vec3(0.6, 0.5, 0.7) - p)/distance(p, vec3(0.6, 0.5, 0.7));
    }
}

vec3 light(vec3 p) {
    vec3 LIGHT_SOURCE = normalize(vec3(5., 10., 1.));
...
//        return vec3(d)/10.;
        if (d < 0.00005) {
            return clamp(vec3(0.1), vec3(dot(dirLight(curr),normal(curr))), vec3(1.));
        } else {
         s += d;
        }
    }
